...
function isAlphanumericBitwise(char: string): boolean {
    ...
    return ((code - 48) & 0xF0) === 0 && (code - 48) <= 9 ||  // 0-9
           ((code - 65) & 0xE0) === 0 && (code - 65) <= 25 || // A-Z
           ((code - 97) & 0xE0) === 0 && (code - 97) <= 25;   // a-z
}
...
testCases.forEach(test => console.log(`  "${test}": ${isPalindromePreprocess(test)}`));

console.log("Optimized:");
testCases.forEach(test => console.log(`  "${test}": ${isPalindromeOptimized(test)}`));
...
const largeNonPalindrome = "a".repeat(1000) + "b".repeat(1000) + "c".repeat(1000);
